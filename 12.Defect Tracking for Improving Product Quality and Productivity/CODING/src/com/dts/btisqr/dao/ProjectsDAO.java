...
//get Free Members 
	...
	public CoreHash getFreeMemebers(String role)
	{
		...
		try
		{
	        ...
	        ResultSet rs = st.executeQuery("select loginname from logindetails ld where logintype='"+role+"'");
	...
	public CoreHash getProject(String loginname)
	{
		//Project aProject = new Project();
		CoreHash aCoreHash = new CoreHash();
		 int i = 0;	
		try
		{
			...
			while(rs.next())
			{
				...
				aProject.setProjectID(rs.getInt(1));
				System.out.println("abbbb"+rs.getInt(1));
				aProject.setProjectName(rs.getString(2));
				System.out.println("ccccc"+rs.getString(2));
				aCoreHash.put(new Integer(abc), aProject);
			}
		}
		catch(Exception e)
		{
